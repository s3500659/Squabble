...
namespace Squabble.Managers
{
    public class ChannelManager : IDataRepo<Channel, int>
    {
        ...
        public async Task<List<ChannelMemberInfo>> GetMemberAcsIds(int id)
        {
            var server = _context.Channels.FirstOrDefault(x => x.ChannelId == id);
            ...
            var channelMemberInfos = new List<ChannelMemberInfo>();
            ...
            {
                string role = null;
                var admin = admins.FirstOrDefault(admin => admin.UserId == ac.AccountId);
                if (admin != null) role = "Admin";
                if (admin == null && ac.AccountId == owner.UserId) role = "Owner";
                if (admin == null && role == null) role = "Member";
        ...
        {
            if (_context.ChannelMembers.Any(x => x.ChannelId == member.ChannelId && x.UserID == member.UserID))
            {
                _logger.LogInformation($"{member.UserID} already exists in {member.ChannelId}");
                return null;
            }
        ...
        public async Task RemoveUser(ChannelMember member)
        {
            try
            {
        ...
        {
            return _context.Channels.FirstOrDefault(x => x.ChannelId == id);
        }
    }
}
